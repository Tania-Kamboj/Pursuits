...
export const DegreeProgramsPage = () => {
  ...
  // ✅ UseActiveStream hook se streamId lo
    ...
    const fetchDegrees = async () => {
      try {
        ...
        const response = await fetch(
          `${API_URL}/degrees?category=${categoryId}&stream=${streamId}`,
        );
        ...
        <main className="flex-1 min-w-0 pb-20">
          ...
          <div className="flex items-center justify-between mb-8 sticky top-[90px] z-40 backdrop-blur-md py-4 px-6 -mx-6 ">
            <div className="flex items-center gap-2 text-body-md text-on-surface-variant">
              ...
              <ChevronRight size={16} className="text-outline-variant" />
              <Link
                to={`/explore/after-12th/degree-categories/${streamId}`}
                className="hover:text-[#3DC6E7] transition-colors"
              >
                Degree Categories
              </Link>
              <ChevronRight size={16} className="text-outline-variant" />
              <span className="text-[#3DC6E7] font-semibold truncate max-w-[300px]">
                {categoryName}
              </span>
            </div>
            ...
            <button
              onClick={() =>
                navigate(`/explore/after-12th/degree-categories/${streamId}`)
              }
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {degrees.map((degree) => (
                <div
                  key={degree._id}
                  className="bg-surface-container-lowest rounded-2xl p-8 border border-outline-variant/20 hover:border-[#3DC6E7]/30 hover:shadow-card-hover transition-all duration-300 flex flex-col"
                >
                  {/* Header with Icon and Duration */}
                  <div className="flex items-start justify-between mb-6">
                    <div className="w-12 h-12 rounded-xl bg-[#3DC6E7]/10 flex items-center justify-center">
                      <GraduationCap className="text-[#3DC6E7]" size={24} />
                    </div>
                    <WishlistButton
                      id={degree._id}
                      type="degree" // ✅ Ye sahi hai!
                      name={degree.name}
                      stream={streamId}
                  ...
                  <Link
                    ...
                    className="inline-flex items-center justify-center gap-2 w-full py-3 rounded-xl text-[#3DC6E7] font-semibold text-[15px] border-2 border-[#3DC6E7]/20 hover:border-[#3DC6E7] hover:bg-[#3DC6E7] hover:text-white transition-all mt-auto"
                  >
                    View Curriculum
                    <ArrowRight size={18} />
                  </Link>
