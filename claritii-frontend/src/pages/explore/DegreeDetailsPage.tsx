...
import { useActiveStream } from '@/hooks/useActiveStream' // ✅ Global hook use kar rahe hain
...
export const DegreeDetailsPage = () => {
  ...
  // ✅ useActiveStream se stream lo, taaki undefined na ho
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-background">
        <div className="text-center">
          <p className="text-xl text-on-surface-variant mb-4">Degree not found.</p>
          <button 
            onClick={() => navigate(`/explore/after-12th/${streamId}/degrees/category/${categoryId}`)}
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <div className="flex flex-1 w-full max-w-[1400px] mx-auto px-8 py-10 gap-10">
        ...
        {/* ✅ Sidebar - Graduation Active */}
        <DashboardSidebar activeTab="graduation" />
        ...
        <main className="flex-1 min-w-0 pb-20">
          ...
          <div className="flex items-center justify-between mb-8 sticky top-[90px] z-40 backdrop-blur-md py-4 px-6 -mx-6 ">
            <div className="flex items-center gap-2 text-body-md text-on-surface-variant">
              ...
              <ChevronRight size={16} className="text-outline-variant" />
              <Link to={`/explore/after-12th/dashboard/${streamName.toLowerCase()}`} className="hover:text-[#3DC6E7] transition-colors">{streamName}</Link>
              <ChevronRight size={16} className="text-outline-variant" />
              <Link to={`/explore/after-12th/${streamId}/degrees/category/${categoryId}`} className="hover:text-[#3DC6E7] transition-colors">Degrees</Link>
              <ChevronRight size={16} className="text-outline-variant" />
              <span className="text-[#3DC6E7] font-semibold truncate max-w-[200px]">{degree.name}</span>
            </div>
          ...
          {/* Header Section (Image & Fee/Salary removed) */}
          <div className="mb-10">
            ...
            {/* Duration Badge (Top Card ki jagah) */}
            <div className="inline-flex items-center gap-3 px-5 py-3 rounded-xl bg-surface-container-lowest border border-outline-variant/20 shadow-sm mt-2">
              ...
              <div>
                <p className="text-xs text-on-surface-variant font-medium">Duration & Format</p>
                <p className="text-sm font-bold text-on-surface">{degree.duration}</p>
              </div>
            </div>
          </div>

          {/* Details Grid (Matching Image Layout) */}
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            
            {/* 1. Core Subjects (Left Column) */}
            <div className="bg-surface-container-lowest rounded-2xl p-8 border border-outline-variant/20">
              <div className="flex items-center gap-3 mb-6">
                ...
                <h2 className="text-xl font-bold text-on-surface">Core Subjects</h2>
              </div>
              <ul className="space-y-4">
                {degree.coreSubjects.map((subject, idx) => (
                  <li key={idx} className="flex items-start gap-3 text-body-md text-on-surface">
                    <CheckCircle2 className="text-[#3DC6E7] flex-shrink-0 mt-0.5" size={18} />
                    <span>{subject}</span>
                  </li>
                ))}
              </ul>
            ...
            {/* 2. Top Career Options (Right Column) */}
            <div className="bg-surface-container-lowest rounded-2xl p-8 border border-outline-variant/20">
              <div className="flex items-center gap-3 mb-6">
                <div className="w-10 h-10 rounded-xl bg-[#3DC6E7]/10 flex items-center justify-center">
                  <Briefcase className="text-[#3DC6E7]" size={20} />
                </div>
                <h2 className="text-xl font-bold text-on-surface">Top Career Options</h2>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {degree.careers.map((career, idx) => (
                  ...
                  >
                    ...
                    <span className="text-sm font-medium text-on-surface">{career}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="bg-surface-container-lowest rounded-2xl p-8 border border-outline-variant/20">
              <div className="flex items-center gap-3 mb-6">
                ...
                <h2 className="text-xl font-bold text-on-surface">Entrance Exams</h2>
              </div>
            ...
            <div className="bg-surface-container-lowest rounded-2xl p-8 border border-outline-variant/20">
              <div className="flex items-center gap-3 mb-6">
                ...
                <h2 className="text-xl font-bold text-on-surface">Best if you are interested in</h2>
              </div>
              <div className="flex flex-wrap gap-3">
                {degree.interests.map((interest, idx) => (
                  <span 
                    ...
                    style={{ backgroundColor: '#5DD4EF3A', color: '#5DD4EF', border: '2px solid rgba(70, 72, 212, 0.2)' }}
                  >
                    {interest}
                  </span>
                ))}
