...
import { auth } from "@/config/firebase"; // ✅ Ensure ye path sahi hai
import { Button } from "./Button";
import { motion, AnimatePresence } from "framer-motion";
...
export const Navbar = () => {
  ...
  useEffect(() => {
    ...
    // ✅ FIREBASE AUTH STATE LISTENER (Ye magic hai!)
    const unsubscribe = onAuthStateChanged(auth, (firebaseUser) => {
      if (firebaseUser) {
        // Firebase user object ko tumhare UI ke format me convert karna
        setUser({
          name: firebaseUser.displayName || firebaseUser.email?.split('@')[0] || "User",
          email: firebaseUser.email,
          uid: firebaseUser.uid
        });
        // Optional: LocalStorage bhi update kar do consistency ke liye
  ...
  // ✅ Agar loading ho raha hai toh ek chota sa spinner dikhao (flicker rokhne ke liye)
  if (loading) {
    return (
